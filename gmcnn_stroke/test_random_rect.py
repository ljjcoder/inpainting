...
masked_root="/home/lijunjie/generative_inpainting-master_global_local/data/random_rec_places_10000/"

if os.path.isfile(masked_root):
    pathfile = open(masked_root, 'rt').read().splitlines()
elif os.path.isdir(masked_root):
    pathfile = glob.glob(os.path.join(masked_root, '*.png'))
else:
...
with tf.Session(config=sess_config) as sess:
    ...
    for i in range(test_num):
        if config.mask_type == 'rect':
            ...
            mask=(cv2.imread(pathfile[i])/255). astype(np.float32)[:,:,0:1]
            #print(mask.shape)
            #exit(0)
        else:
        ...
        image = cv2.imread(config.dataset_path+name)
        #index=pathfile[i].rfind('/')
        #name=pathfile[i][index+1:len(pathfile[i])]
        ...
        if i==0:
            cv2.imwrite(os.path.join(config.save_root2, '0input_'+name), image.astype(np.uint8))
