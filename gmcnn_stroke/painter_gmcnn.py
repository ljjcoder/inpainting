...
class Paint(object):
    ...
    def __init__(self, config):
        ...
        self.c.create_text(250, 50, text='Input')
        ...
        self.out1 = Canvas(self.root, bg='white', width=config.img_shapes[1]+8, height=config.img_shapes[0])
        self.out1.grid(row=0, column=2, rowspan=8)
    ...
    def load(self):
        ...
        try:
            ...
            photo = photo.crop([0,0,self.config.img_shapes[1],self.config.img_shapes[0]])
            
            self.image = cv2.imread(self.filename)[:self.config.img_shapes[0], :self.config.img_shapes[1],:]
        except:
            print('do not load image')
        else:
            self.im_w, self.im_h = photo.size
            self.mask = np.zeros((self.im_h, self.im_w, 1)).astype(np.uint8)[:self.config.img_shapes[0], :self.config.img_shapes[1],0:1] 
            #self.mask = cv2.imread("/home/lijunjie/inpainting_gmcnn-master/tensorflow_MLD/imgs/places2-512x680/Places365_val_00006822_mask1.png")[:self.config.img_shapes[0], :self.config.img_shapes[1],0:1]           
            print(photo.size,self.mask.shape)
    ...
    def save(self):
        ...
        cv2.imwrite(os.path.join(self.filepath, self.filename_+'_masked.png'), self.result[0][:, :, ::-1]*(1-self.mask[:,:,:])+self.mask[:,:,:]*255)        
        cv2.imwrite(os.path.join(self.filepath, self.filename_ + '_gm_result.png'), self.result[0][:, :, ::-1])
        cv2.imwrite(os.path.join(self.filepath, self.filename_ + '_gm_result_mld.png'), self.result1[0][:, :, ::-1])
    def fill(self):
        if self.mode == 'rect':
            for rect in self.mask_candidate:
                self.mask[rect[1]:rect[3], rect[0]:rect[2], :] = 1
        ...
        assign_ops=[]
        assign_ops = list(map(lambda x: tf.assign(x, tf.contrib.framework.load_variable(config.load_model_dir1, x.name)),
                              self.vars_list))
        self.sess.run(assign_ops)    
        self.result1 = self.sess.run(self.output, feed_dict={self.input_image_tf: image * 1.0,
                                                            self.input_mask_tf: mask * 1.0})        
        cv2.imwrite('./imgs/tmp.png', self.result[0][:, :, ::-1])
        cv2.imwrite('./imgs/tmp1.png', self.result1[0][:, :, ::-1])
        ...
        photo1 = Image.open('./imgs/tmp1.png')
        self.displayPhotoResult1 = photo1
        self.displayPhotoResult1 = self.displayPhotoResult1.resize((self.im_w, self.im_h))
        self.photo_tk_result1 = ImageTk.PhotoImage(image=self.displayPhotoResult1)
