...
from net.ops import id_mrf_reg,littilewave,style_loss
    ...
    def build_net(self, batch_data, config, summary=True, reuse=False):
        ...
        if not config.pretrain_network:
            ...
            if config.use_style_loss:
                ID_MRF_loss,vgg = id_mrf_reg(local_patch_batch_pred, local_patch_batch_pos, config)
                style_loss_predicted=style_loss(batch_predicted, batch_pos,vgg)
                #style_loss_com=style_loss(batch_complete, batch_pos,vgg)
        ...
        if not config.pretrain_network:
            ...
            if config.use_style_loss:
                losses['l1_loss'] += 120*(style_loss_predicted)
