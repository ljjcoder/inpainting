...
class GMCNNModel:
    ...
    def build_net(self, batch_data, config, summary=True, reuse=False):
        ...
        losses['l1_loss_only'] =tf.reduce_mean(tf.abs(local_patch_batch_pos - local_patch_batch_pred))            
        if not config.pretrain_network:
        ...
        for i in range(len(a)):
            ...
            #print('ppppppppppppp,network_test_wt0.1')
            #exit(0)                
        ...
        if summary:
            ...
            tf.summary.scalar('losses/l1_loss_only', losses['l1_loss_only'])
        #print('ljj')    
        #exit(0)
        # gan
        ...
        if summary and not config.pretrain_network:
            tf.summary.scalar('convergence/d_loss', losses['d_loss']/2) 
        losses['d_loss_nogp'] =losses['d_loss']          
        losses['d_loss'] = losses['d_loss'] + losses['gp_loss']
        if summary and not config.pretrain_network:
            tf.summary.scalar('convergence/d_loss_gp', losses['d_loss']/2)
    ...
    def evaluate(self, im, mask, config, reuse=False):
        ...
        #l1_loss= tf.reduce_mean(tf.abs(batch_complete[0,64:192,64:192,0] - im_pos[0,64:192,64:192,0]))
        l1_loss= tf.reduce_mean(tf.abs(batch_complete - im_pos))        
        #tf.summary.scalar('l1_test', l1_loss)
        return batch_complete,l1_loss
