...
class GMCNNModel:
    ...
    def build_net(self, batch_data, config, summary=True, reuse=False):
        ...
        if config.mask_type == 'rect':
            ...
            mask = free_form_mask_tf(parts=4, im_size=(config.img_shapes[0], config.img_shapes[1]),
                                     maxBrushWidth=40, maxLength=80, maxVertex=20)
        #print(type(batch_pos),type((1. - mask)))
        ...
        if summary and not config.pretrain_network:
            tf.summary.scalar('convergence/d_loss_gp', losses['d_loss'])
