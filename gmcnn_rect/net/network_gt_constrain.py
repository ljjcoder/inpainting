...
class GMCNNModel:
    ...
    def build_generator(self, x, mask, reuse=False, name='inpaint_net',stop_grad=False):
    ...
    def build_net(self, batch_data, config, summary=True, reuse=False):
        ...
        batch_predicted,features = self.build_generator(batch_incomplete, mask, reuse=reuse)
        _,features_gt = self.build_generator(batch_pos, mask-mask, reuse=True,stop_grad=True) 
        features_loss=0
        for i in range(len(features)):
            features_loss =features_loss+ tf.reduce_mean(tf.abs(features_gt[i]-features[i]))                   
        ...
        losses['g_loss'] += config.ae_loss_alpha * losses['ae_loss']+self.harr_loss_x2_local+self.harr_loss_x2+features_loss
