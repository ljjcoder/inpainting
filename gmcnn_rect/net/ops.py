...
def bbox2mask_board_random(bbox,h,w, config, name='mask'):
    """Generate mask tensor from bbox.
    ...
    def npmask(bbox, height, width, h, w):
        ...
        #h = np.random.randint(delta_h//2+1)
        #w = np.random.randint(delta_w//2+1)
        bored_h=np.random.randint(np.int32((bbox[2]-2*h)/8),np.int32((bbox[2]-2*h)/4))
        bored_w=np.random.randint(np.int32((bbox[3]-2*w)/8),np.int32((bbox[2]-2*w)/4))
        random_h=np.random.randint(4,16)
        random_w=np.random.randint(4,16)
        mask[:, bbox[0]+h+bored_h:bbox[0]+bbox[2]-h-bored_h-random_h,
             bbox[1]+w+bored_w:bbox[1]+bbox[3]-w-bored_w-random_w, :] = 1.
        return mask
    with tf.variable_scope(name), tf.device('/cpu:0'):
        img_shape = config.img_shapes
        height = img_shape[0]
...
def gan_log_loss(pos, neg, name='gan_log_loss'):
    ...
    with tf.variable_scope(name):
        ...
        tf.summary.scalar('d_scores/pos_mean', pos_acc)
        tf.summary.scalar('d_scores/neg_mean', neg_acc)
        # loss
...
class Vgg19(object):
    def __init__(self, filepath=None):
        ...
        self.vgg_weights = filepath if filepath is not None else os.path.join('/home/lijunjie/inpainting_gmcnn-master/tensorflow_MLD/vgg19_weights', 'imagenet-vgg-verydeep-19.mat')
        #print(self.vgg_weights,filepath)
        #exit(0)
