...
from net.ops import gan_wgan_loss, gradients_penalty, random_interpolates,gan_log_loss
...
class GMCNNModel:
    ...
    def wgan_global_discriminator(self, x, d_cnum, reuse=False):
        ...
        with tf.variable_scope('disc_global', reuse=reuse):
            ...
            #exit(0)
            return x
    def wgan_local_discriminator_spetral(self, x, d_cnum, reuse=False):
        cnum = d_cnum
        with tf.variable_scope('disc_local', reuse=reuse):
            x = self.conv5_ds(x, filters=cnum, name='conv1')
            x = self.conv5_ds(x, filters=cnum * 2, name='conv2')
    ...
    def wgan_global_discriminator_spetral(self, x, d_cnum, reuse=False):
        cnum = d_cnum
        with tf.variable_scope('disc_global', reuse=reuse):
            x = self.conv5_ds(x, filters=cnum, name='conv1')
            x = self.conv5_ds(x, filters=cnum * 2, name='conv2')
    ...
    def wgan_discriminator_spetral(self, batch_local, batch_global, d_cnum, reuse=False):
        with tf.variable_scope('discriminator', reuse=reuse):
            dlocal = self.wgan_local_discriminator_spetral(batch_local, d_cnum, reuse=reuse)
            dglobal = self.wgan_global_discriminator_spetral(batch_global, d_cnum, reuse=reuse)
            dout_local = tf.layers.dense(dlocal, 1, name='dout_local_fc')
            dout_global = tf.layers.dense(dglobal, 1, name='dout_global_fc')
            return dout_local, dout_global
            
    def wgan_discriminator_spetral_bord(self, batch_local, batch_global, d_cnum, reuse=False):
        with tf.variable_scope('discriminator_bord', reuse=reuse):
            dlocal = self.wgan_local_discriminator_spetral(batch_local, d_cnum, reuse=reuse)
            dglobal = self.wgan_global_discriminator_spetral(batch_global, d_cnum, reuse=reuse)
            dout_local = tf.layers.dense(dlocal, 1, name='dout_local_fc')
    ...
    def build_net(self, batch_data, config, summary=True, reuse=False):
        ...
        for i in range(len(a)):
            if ((i%4)==0)&(i>0):
                continue
            if (i==0):                    
                self.harr_loss_x2=self.harr_loss_x2+0.1*tf.reduce_mean(tf.square(a[i]-b[i]))
            else:
        ...
        for i in range(len(a_local)):
            if ((i%4)==0)&(i>0):
                continue
            if (i==0):                
                self.harr_loss_x2_local=self.harr_loss_x2_local+0.1*tf.reduce_mean(tf.square(a_local[i]-b_local[i]))
            else: 
        ...
        if config.mask_type == 'rect':
            ...
            pos_neg_local_center, pos_neg_global_center = self.wgan_discriminator_spetral(local_patch_batch_pos_neg_center,
            ...
            # wgan with gradient penalty
            pos_neg_local_bord, pos_neg_global_bord = self.wgan_discriminator_spetral_bord(local_patch_batch_pos_neg_bord,
        ...
        g_loss_local_center, d_loss_local_center = gan_log_loss(pos_local_center, neg_local_center, name='gan/local_gan_center')
        g_loss_global_center, d_loss_global_center = gan_log_loss(pos_global_center, neg_global_center, name='gan/global_gan_center')
        g_loss_local_bord, d_loss_local_bord = gan_log_loss(pos_local_bord, neg_local_bord, name='gan/local_gan_bord')
        g_loss_global_bord, d_loss_global_bord = gan_log_loss(pos_global_bord, neg_global_bord, name='gan/global_gan_bord')        
