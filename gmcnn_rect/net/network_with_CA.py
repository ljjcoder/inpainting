...
from net.ops_CA import contextual_attention,resize_mask_like,Aspp,contextual_attention_max
...
class GMCNNModel:
    ...
    def build_generator(self, x, mask, reuse=False, name='inpaint_net'):
        ...
        with tf.variable_scope(name, reuse=reuse):
            ...
            x_pure_7=x 
            mask_s_7 = resize_mask_like(mask, x)
            print(x)
            
            # add contextual attention by ljj
            print(self.config.use_CA_max)
            #exit(0)
            if self.config.use_CA_max:
                x, offset_flow = contextual_attention_max(x, x, mask_s_7, 3, 1, rate=2)
                print('ca_max')
            else:
                x, offset_flow = contextual_attention(x, x, mask_s_7, 3, 1, rate=2)
                print('use ca_ori')
            #exit(0)
            #x, offset_flow = contextual_attention_max(x, x, mask_s_7, 3, 1, rate=2)
            x = self.Aspp_1.get_fuse_feature(x,b_names[0] + 'CA_conv1')
            x_ca_merge_7 = tf.concat([x_pure_7, x], axis=3)
            x_RGB_b1 = tf.layers.conv2d(inputs=x_ca_merge_7, kernel_size=3, filters=3, strides=1, activation=None, padding='SAME',
                                 name=b_names[3] + 'CA_RGB_conv18_b1')
            x_RGB_b1 = tf.clip_by_value(x_RGB_b1, -1., 1.)                                 
            x = conv_7(inputs=x_ca_merge_7, filters=4 * cnum, strides=1, name=b_names[0] + 'CA_conv13')
            #contextual end            
            ...
            x_pure_5=x 
            mask_s_5 = resize_mask_like(mask, x)
            print(x)            
            # add contextual attention by ljj
            if self.config.use_CA_max:    
                x, offset_flow = contextual_attention_max(x, x, mask_s_5, 3, 1, rate=2)
            else:                
                x, offset_flow = contextual_attention(x, x, mask_s_5, 3, 1, rate=2)
            #x, offset_flow = contextual_attention_max(x, x, mask_s_5, 3, 1, rate=2)
            x =self.Aspp_2.get_fuse_feature(x, b_names[1] + 'CA_conv1')
            x_ca_merge = tf.concat([x_pure_5, x], axis=3)
            x_RGB_b2 = tf.layers.conv2d(inputs=x_ca_merge, kernel_size=3, filters=3, strides=1, activation=None, padding='SAME',
                                 name=b_names[3] + 'CA_RGB_conv18_b2')   
            x_RGB_b2 = tf.clip_by_value(x_RGB_b2, -1., 1.)                                 
            #contextual end
            x = tf.image.resize_nearest_neighbor(x_ca_merge, [xh//2, xw//2], align_corners=True)
            with tf.variable_scope(b_names[1] + 'conv13_upsample'):
                x = conv_3(inputs=x, filters=2 * cnum, strides=1, name=b_names[1] + 'CA_conv13_upsample_conv')
            ...
            x_pure_3=x 
            print(x)
            mask_s_3 = resize_mask_like(mask, x)            
            #exit(0)            
            # add contextual attention by ljj
            if self.config.use_CA_max:
                x, offset_flow = contextual_attention_max(x, x, mask_s_3, 3, 1, rate=2) 
            else:                
                x, offset_flow = contextual_attention(x, x, mask_s_3, 3, 1, rate=2)
            #x, offset_flow = contextual_attention_max(x, x, mask_s_3, 3, 1, rate=2)
            x = self.Aspp_3.get_fuse_feature(x, b_names[2] + 'CA_conv1')
            x_ca_merge_3 = tf.concat([x_pure_3, x], axis=3)
            x_RGB_b3 = tf.layers.conv2d(inputs=x_ca_merge_3, kernel_size=3, filters=3, strides=1, activation=None, padding='SAME',
                                 name=b_names[3] + 'CA_RGB_conv18_b3')
            x_RGB_b3 = tf.clip_by_value(x_RGB_b3, -1., 1.)                                 
            #contextual end              
            x = tf.image.resize_nearest_neighbor(x_ca_merge_3, [xh // 2, xw // 2], align_corners=True)
            with tf.variable_scope(b_names[2] + 'conv13_upsample'):
                x = conv_3(inputs=x, filters=2 * cnum, strides=1, name=b_names[2] + 'CA_conv13_upsample_conv')
            ...
            #exit(0)
            # add contextual attention by ljj
            #x, offset_flow = contextual_attention(x, x, mask, 3, 1, rate=2)
            #x = tf.layers.conv2d(inputs=x, kernel_size=3, filters=cnum // 2, strides=1, activation=tf.nn.elu, padding='SAME',
                                 #name=b_names[3] + 'CA_conv1')
            #x = tf.layers.conv2d(inputs=x, kernel_size=3, filters=cnum // 2, strides=1, activation=tf.nn.elu, padding='SAME',
                                 #name=b_names[3] + 'CA_conv2')
            #x = tf.layers.conv2d(inputs=x, kernel_size=3, filters=cnum // 2, strides=1, activation=tf.nn.elu, padding='SAME',
                                 #name=b_names[3] + 'CA_conv3') 
            #x_ca_merge = tf.concat([x_pure, x], axis=3)
            #contextual end
                                 
            #x = tf.layers.conv2d(inputs=x_ca_merge, kernel_size=3, filters=3, strides=1, activation=None, padding='SAME',
                                 #name=b_names[3] + 'CA_conv18')            
            x = tf.layers.conv2d(inputs=x, kernel_size=3, filters=3, strides=1, activation=None, padding='SAME',
                                 name=b_names[3] + 'conv18')
            x = tf.clip_by_value(x, -1., 1.)
        return x,x_RGB_b1,x_RGB_b2,x_RGB_b3
    ...
    def build_net(self, batch_data, config, summary=True, reuse=False):
        ...
        print(batch_pos)
        #exit(0)
        pos_small=tf.image.resize_bilinear(batch_pos, [64, 64],align_corners=False)

        #exit(0)
        # generate mask, 1 represents masked point
        if config.mask_type == 'rect':
            bbox = random_bbox(config)
            bbox_small=(tf.cast(bbox[0]/4,tf.int32),tf.cast(bbox[1]/4,tf.int32),tf.cast(bbox[2]/4,tf.int32),tf.cast(bbox[3]/4,tf.int32))
            #print(bbox,bbox_small)
            #exit(0)
            mask,h,w = bbox2mask(bbox, config, name='mask_c')
            mask_small=resize_mask_like(mask, pos_small)
            ...
            mask,mask_center = free_form_mask_tf(parts=4, im_size=(config.img_shapes[0], config.img_shapes[1]),
                                     maxBrushWidth=40, maxLength=80, maxVertex=20)
            mask_bord=mask-mask_center
        batch_incomplete = batch_pos * (1. - mask)
        batch_incomplete_small=pos_small * (1. - mask_small)
        mask_priority = priority_loss_mask(mask)
        mask_priority_small= priority_loss_mask(mask_small)
        #print(self.config)
        #exit(0)
        batch_predicted,x_RGB_b1,x_RGB_b2,x_RGB_b3 = self.build_generator(batch_incomplete, mask, reuse=reuse)
        #print(x_RGB_b2.shape,x_RGB_b1.shape,x_RGB_b3.shape)
        #exit(0)

        losses = {}
        # apply mask and complete image
        batch_complete = batch_predicted * mask + batch_incomplete * (1. - mask)
        batch_complete_small_1 = x_RGB_b1 * mask_small + batch_incomplete_small * (1. - mask_small)
        batch_complete_small_2 = x_RGB_b2 * mask_small + batch_incomplete_small * (1. - mask_small)  
        batch_complete_small_3 = x_RGB_b3 * mask_small + batch_incomplete_small * (1. - mask_small)        
        #added by ljj
        ...
        if config.mask_type == 'rect':
            ...
            #added by ljj
            local_patch_batch_pos_small = local_patch(pos_small, bbox_small)            
            local_patch_batch_complete_small_1 = local_patch(batch_complete_small_1, bbox_small)
            local_patch_batch_complete_small_2 = local_patch(batch_complete_small_2, bbox_small)    
            local_patch_batch_complete_small_3 = local_patch(batch_complete_small_3, bbox_small)    
            ...
            local_patch_batch_pred_small_1 = local_patch(x_RGB_b1, bbox_small)
            local_patch_batch_pred_small_2 = local_patch(x_RGB_b2, bbox_small)    
            local_patch_batch_pred_small_3 = local_patch(x_RGB_b3, bbox_small)
            
            mask_priority = local_patch(mask_priority, bbox)
            mask_priority_samll = local_patch(mask_priority_small, bbox_small)            
        else:
        ...
        losses['l1_loss'] = \
            ...
            pretrain_l1_alpha*0.00001 * tf.reduce_mean(tf.abs(local_patch_batch_pos_small - local_patch_batch_pred_small_1) * mask_priority_samll)+\
            pretrain_l1_alpha*0.00001 * tf.reduce_mean(tf.abs(local_patch_batch_pos_small - local_patch_batch_pred_small_2) * mask_priority_samll)+\
            pretrain_l1_alpha*0.00001 * tf.reduce_mean(tf.abs(local_patch_batch_pos_small - local_patch_batch_pred_small_3) * mask_priority_samll)
        if not config.pretrain_network:
        ...
        if not config.pretrain_network:
            losses['ae_loss'] += pretrain_l1_alpha * tf.reduce_mean(tf.abs(batch_pos - batch_predicted) * (1. - mask))+\
             pretrain_l1_alpha*0.00001 * tf.reduce_mean(tf.abs(pos_small - x_RGB_b1) * (1. - mask_small))+\
              pretrain_l1_alpha*0.00001 * tf.reduce_mean(tf.abs(pos_small - x_RGB_b2) * (1. -  mask_small))+\
               pretrain_l1_alpha*0.00001 * tf.reduce_mean(tf.abs(pos_small - x_RGB_b3) * (1. -  mask_small))
        ...
        for i in range(len(a)):
            if (i%4)==0:
                continue	
            self.harr_loss_x2=self.harr_loss_x2+tf.reduce_mean(tf.square(a[i]-b[i]))
        ...
        for i in range(len(a_local)):
            if (i%4)==0:
                continue		
            self.harr_loss_x2_local=self.harr_loss_x2_local+tf.reduce_mean(tf.square(a_local[i]-b_local[i]))
            #self.harr_loss_x1_local=self.harr_loss_x1_local+tf.reduce_mean(tf.square(c_local[i]-b_local[i])) 
        #added end local
        if summary:
            viz_img = tf.concat([batch_pos, batch_incomplete, batch_predicted, batch_complete_bord], axis=2)[:, :, :, ::-1]
        ...
        penalty_global_center = gradients_penalty(interpolates_global_center, dout_global_center, mask=mask)
        penalty_global_bord = gradients_penalty(interpolates_global_bord, dout_global_bord, mask=mask)        
        ...
        losses['g_loss'] += config.ae_loss_alpha * losses['ae_loss']+self.harr_loss_x2_local+self.harr_loss_x2
        #losses['g_loss'] += config.ae_loss_alpha * losses['ae_loss']+self.harr_loss_x2
    ...
    def evaluate(self, im, mask, config, reuse=False):
        ...
        im = im * (1 - mask)
        # inpaint
        batch_predict,_,_,_ = self.build_generator(im, mask, reuse=reuse)
        #print(batch_predict)
        #print(mask)
        #exit(0)
