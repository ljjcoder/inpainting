...
class GMCNNModel:
    ...
    def wgan_discriminator_bord(self, batch_local, batch_global, d_cnum, reuse=False):
        with tf.variable_scope('discriminator_bord', reuse=reuse):
            ...
            dout_global = tf.layers.dense(dglobal, 1, name='dout_global_fc')
            return dout_local, dout_global
    def wgan_discriminator_whole(self, batch_local, batch_global, d_cnum, reuse=False):
        with tf.variable_scope('discriminator_whole', reuse=reuse):
            dlocal = self.wgan_local_discriminator(batch_local, d_cnum, reuse=reuse)
            dglobal = self.wgan_global_discriminator(batch_global, d_cnum, reuse=reuse)
            dout_local = tf.layers.dense(dlocal, 1, name='dout_local_fc')
            dout_global = tf.layers.dense(dglobal, 1, name='dout_global_fc')
    ...
    def build_net(self, batch_data, config, summary=True, reuse=False):
        ...
        if config.mask_type == 'rect':
            ...
            # wgan with gradient penalty
            pos_neg_local, pos_neg_global = self.wgan_discriminator_whole(local_patch_batch_pos_neg,
        ...
        losses['g_loss'] = global_wgan_loss_alpha * (g_loss_global_center+g_loss_global_bord+g_loss_global) + g_loss_local_center+g_loss_local_bord+g_loss_local
        losses['d_loss'] = d_loss_global_center + d_loss_local_center+d_loss_global_bord + d_loss_local_bord+d_loss_global + d_loss_local
        # gp
        ...
        losses['gp_loss'] = config.wgan_gp_lambda * (penalty_local_center + penalty_global_center+penalty_global)+config.wgan_gp_lambda * (penalty_local_bord + penalty_global_bord+penalty_local)
        ...
        losses['g_loss'] += config.ae_loss_alpha * losses['ae_loss']+self.harr_loss_x2+self.harr_loss_x2_local
