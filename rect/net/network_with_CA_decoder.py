...
class GMCNNModel:
    ...
    def build_generator(self, x, mask, reuse=False, name='inpaint_net'):
        ...
        with tf.variable_scope(name, reuse=reuse):
            ...
            merge_shape=x_merge.get_shape().as_list()
            #print(merge_shape,'ppppppppp')
            #exit(0)
            x_resize = tf.image.resize_nearest_neighbor(x_merge, [xh // 4, xw // 4], align_corners=True)   
            #x, offset_flow = contextual_attention(x_resize, x_resize, mask_s_7, 3, 1, rate=2)
            
            mask_s_7 = resize_mask_like(mask, x_resize)
            
            x_ca, offset_flow = contextual_attention(x_resize, x_resize, mask_s_7, 3, 1, rate=2)
            x_ca =self.Aspp_1.get_fuse_feature(x_ca, b_names[0] + 'CA_conv1')
            x_ca = tf.image.resize_nearest_neighbor(x_ca, [xh, xw], align_corners=True)
            x_ca = tf.layers.conv2d(inputs=x_ca, kernel_size=3, filters=merge_shape[3], strides=1, activation=None, padding='SAME',
                                 name=b_names[0] + 'CA_conv18')
            x_merge_fuse= x_merge+ x_ca                    
                                 
            x = conv_3(inputs=x_merge_fuse, filters=cnum // 2, strides=1, name=b_names[3] + 'conv17')
