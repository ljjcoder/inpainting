...
def littilewave(img1,level):
    imgtemp=tf.reduce_mean(img1,axis=3)
    tensor_shape=img1.get_shape().as_list()
    img= tf.expand_dims(imgtemp,3)
    #print(img,tensor_shape)
    #exit(0)
    with tf.variable_scope("Littlewave"):
        #[L1,LH,HL,HH]=haar_dwt2D(img)
        a=[]
...
def haar_dwt2D(img):
    with tf.variable_scope("haar_dwt2D"):
        Ww=np.zeros([1, 2, 1, 1])
        Ww[0,0]=0.5
        Ww[0,1]=0.5
        Wh=np.zeros([1, 2, 1, 1])
        ...
        Hh[1,0]=-0.5
#do in rows	
        convw = tf.nn.conv2d(img, Ww, strides=[1, 1, 2, 1], padding='SAME')
        convh = tf.nn.conv2d(img, Wh, strides=[1, 1, 2, 1], padding='SAME')
        #print(convw,convh)
        #exit(0)
#do in cols	
        convw1 = tf.nn.conv2d(convw, Hw, strides=[1, 2, 1, 1], padding='SAME')
        convh1 = tf.nn.conv2d(convw, Hh, strides=[1, 2, 1, 1], padding='SAME')	
        convw2 = tf.nn.conv2d(convh, Hw, strides=[1, 2, 1, 1], padding='SAME')
        convh2 = tf.nn.conv2d(convh, Hh, strides=[1, 2, 1, 1], padding='SAME')
        #print(convw1,convh1)	
        #exit(0)
    return convw1,convh1,convw2,convh2
def np_free_form_mask(maxVertex, maxLength, maxBrushWidth, maxAngle, h, w):
    ...
    for i in range(numVertex):
        ...
        brushWidth = np.random.randint(25, maxBrushWidth + 1) // 2 * 2
...
def free_form_mask_tf(parts, maxVertex=16, maxLength=60, maxBrushWidth=14, maxAngle=360, im_size=(256, 256), name='fmask'):
    ...
    filter=np.zeros([16,16,1]).astype(dtype=np.float32)
    ...
    with tf.variable_scope(name):
        ...
        mask_center=tf.nn.erosion2d(mask,filter,strides=[1,1,1,1],rates=[1,1,1,1],padding="SAME",name=None)
        #print(mask,mask_center)
        #exit(0)
...
def bbox2mask(bbox, config, name='mask'):
    ...
    with tf.variable_scope(name), tf.device('/cpu:0'):
        ...
        mask,h,w = tf.py_func(
            ...
            [tf.float32,tf.int64,tf.int64], stateful=False)
        mask.set_shape([1] + [height, width] + [1])
    return mask,h,w
    
def bbox2mask_board(bbox,h,w, config, name='mask'):
    """Generate mask tensor from bbox.
    ...
    """
    def npmask(bbox, height, width, h, w):
        mask = np.zeros((1, height, width, 1), np.float32)
        #h = np.random.randint(delta_h//2+1)
        #w = np.random.randint(delta_w//2+1)
        bored_h=np.int32((bbox[2]-2*h)/4)
        bored_w=np.int32((bbox[3]-2*w)/4)		
        mask[:, bbox[0]+h+bored_h:bbox[0]+bbox[2]-h-bored_h,
             bbox[1]+w+bored_w:bbox[1]+bbox[3]-w-bored_w, :] = 1.
        return mask
    with tf.variable_scope(name), tf.device('/cpu:0'):
        img_shape = config.IMG_SHAPES
        height = img_shape[0]
        ...
        mask = tf.py_func(
            npmask,
            [bbox, height, width, h, w],
            tf.float32, stateful=False)
        mask.set_shape([1] + [height, width] + [1])
    return mask 
    
def bbox2mask_center(bbox,h,w, config, name='mask'):
    """Generate mask tensor from bbox.
...
def gan_wgan_loss(pos, neg, name='gan_wgan_loss'):
    ...
    """
    with tf.variable_scope(name):
        d_loss = tf.reduce_mean(neg-pos)#+tf.reduce_mean(tf.abs(pos))
        g_loss = -tf.reduce_mean(neg)#+tf.reduce_mean(tf.abs(neg))
...
def compute_gram(x):
    ...
    #f = x.view(b, ch, w * h)
    f = tf.reshape(x,[b,w * h,ch])
    f_T = tf.transpose(f, perm=[0, 2, 1])
    G = tf.matmul(f_T,f) / (h * w * ch)

    return G
def style_loss(img_gt, img_generated,vgg):
    STYLE_LAYERS = ('conv2_2', 'conv3_4', 'conv4_4', 'conv5_2')
    x_vgg= vgg.build_vgg19((img_gt + 1) * 127.5, reuse=True)
    y_vgg= vgg.build_vgg19((img_generated + 1) * 127.5, reuse=True)
    #x_vgg=
    #print(STYLE_LAYERS[0])
    #print(x_vgg[STYLE_LAYERS[0]])
    #exit(0)
    feature1=tf.reduce_mean(tf.abs(compute_gram(x_vgg[STYLE_LAYERS[0]])-compute_gram(y_vgg[STYLE_LAYERS[0]])))
    feature2=tf.reduce_mean(tf.abs(compute_gram(x_vgg[STYLE_LAYERS[1]])-compute_gram(y_vgg[STYLE_LAYERS[1]])))
    feature3=tf.reduce_mean(tf.abs(compute_gram(x_vgg[STYLE_LAYERS[2]])-compute_gram(y_vgg[STYLE_LAYERS[2]])))
    feature4=tf.reduce_mean(tf.abs(compute_gram(x_vgg[STYLE_LAYERS[3]])-compute_gram(y_vgg[STYLE_LAYERS[3]])))
    loss=feature1+feature2+feature3+feature4
    return loss
